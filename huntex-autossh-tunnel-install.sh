...
ExecStart=/bin/bash -lc '/usr/bin/autossh -M 0 -N -p "\${PORT}" -i "\${KEY}" -o BatchMode=yes -o StrictHostKeyChecking=no -o UserKnownHostsFile="\${KNOWN}" -o PreferredAuthentications=publickey -o PubkeyAuthentication=yes -o PasswordAuthentication=no -o KbdInteractiveAuthentication=no -o IdentitiesOnly=yes -o ExitOnForwardFailure=yes -o ServerAliveInterval=15 -o ServerAliveCountMax=3 -o TCPKeepAlive=yes -o ConnectTimeout=7 -o ConnectionAttempts=1 '"'"'-R \${RHOST}:\${RPORT}:\${LHOST}:\${LPORT}'"'"' "\${USER}@\${IP}" >> "\${LOGFILE}" 2>&1'
...
_is_listening_remote() {
  timeout 8 ssh -p "${PORT}" -i "${KEY}" "${USER}@${IP}" \
    ...
    "command -v ss >/dev/null 2>&1 && ss -lntp | grep -q ':443\b'" >/dev/null 2>&1
}
